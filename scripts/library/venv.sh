...
function _install_requirements() {
    ...
    local REQUIREMENTS_FILE_PATH="${ARBORISTA}/requirements/${REQUIREMENTS_FILE_NAME}"
    python3 -m pip install -r "${REQUIREMENTS_FILE_PATH}"
}

function _install_frozen_requirements() {
    local FROZEN_REQUIREMENTS_FILE_NAME="$1"
    local FROZEN_REQUIREMENTS_FILE_PATH="${ARBORISTA}/requirements/frozen/${FROZEN_REQUIREMENTS_FILE_NAME}"
...
function get_venv_path() {
    local VENV_NAME="$1"
    local VENV_PATH="${ARBORISTA}/venvs/${VENV_NAME}"
    echo "${VENV_PATH}"
}
